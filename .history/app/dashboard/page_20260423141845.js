...
Every word you read brings you closer to fluency.`;
  ...
  const recordCurrentWordTiming = () => {
    if (!startTime || !lines[currentLineIndex]) return;
    ...
    setWordTimings(prev => {
      const existingIndex = prev.findIndex(
        (item) => item.line === currentLineIndex && item.word === currentWordIndex
      );
      if (existingIndex !== -1) {
        // Update existing entry
        const updated = [...prev];
        updated[existingIndex] = { ...updated[existingIndex], timeMs };
        return updated;
      } else {
        // Add new entry
        return [...prev, entry];
  ...
  const completeTranslating = () => {
    ...
    const currentEntry = recordCurrentWordTiming();
    const finalTimings = currentEntry ? [...wordTimings, currentEntry] : wordTimings;
  ...
  const advanceWord = () => {
    ...
    const currentLine = lines[currentLineIndex];
    const words = currentLine.split(" ").filter(Boolean);

    recordCurrentWordTiming();
  ...
  const goBackWord = () => {
    ...
    if (currentWordIndex > 0) {
      ...
      const previousLineWords = lines[currentLineIndex - 1].split(" ").filter(Boolean);
      setCurrentLineIndex(currentLineIndex - 1);
  ...
  const totalWords = lines.reduce((sum, line) => sum + line.split(" ").filter(Boolean).length, 0);
  const currentWordPosition = isTranslating
    ? lines.slice(0, currentLineIndex).reduce((sum, line) => sum + line.split(" ").filter(Boolean).length, 0) + currentWordIndex + 1
    : 0;
